...
<html>
    ...
    <body>
        ...
        <div class="accordion-body">
        <div class="grid-row">
        <div class="grid-column"><label for="cidbGrade">CIDB Grade</label>
        <select name="cidbGrade" id="cidbGrade" required>
        ...
        <div class="grid-column"><label for="cidbSpecialization">CIDB Specialisation</label>
            <input type="text" name="cidbSpecialization" id="cidbSpecialization">
        </div>
        </div>
...
The Contractor is required to complete the form by listing all plant and machinery owned by the company, including details such as quantity, brand/model, capacity rating, ownership status, and year of manufacture. <b>The Contractor shall also provide valid calibration certificates for the relevant equipment.</b> The list must be adjusted and expanded, where necessary, to accurately reflect the actual plant and machinery currently owned, hired or available to the Contractor.
        ...
        <h2 class="accordion-header" id="headingEight">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseEight" aria-expanded="false" aria-controls="collapseEight"><label>Part H: Self Declaration</label></button>
        </h2>
        ...
        <!--    auto fill button remove later-->
                ...
                <script>
                    ...
                    function autoFillTestData() {
                        ...
                        // Populate select elements (pick a valid non-empty option)
                        document.querySelectorAll('select').forEach(sel => {
                            if (sel.disabled) return;
                            const opts = Array.from(sel.options).filter(o => o.value !== '');
                            if (opts.length === 0) return;
                            // choose middle option for predictability
                            const pick = opts[Math.floor(opts.length / 2)];
                            sel.value = pick.value;
                            // dispatch change in case any listeners toggle UI
