...
// OTP pending state
$otpPending = false;
if (isset($_SESSION['otp_pending']) && is_array($_SESSION['otp_pending'])) {
	$pending = &$_SESSION['otp_pending'];
	// purge expired or exhausted pending OTPs so the UI doesn't auto-open
	if (isset($pending['expires']) && time() > $pending['expires']) {
			...
			<div class="form-group" style="display:flex;align-items:center;gap:8px;">
				...
				<div id="newPasswordFields" style="display:none;">
					<div class="form-group" style="position:relative;">
						...
						<button type="button" id="toggleNewPassword" tabindex="-1" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);margin-top:10px;background:none;border:none;cursor:pointer;color:#666;font-size:18px;padding:5px;width:auto;height:auto;line-height:1;">👁️</button>
					</div>
					...
					<div class="form-group">
						...
						<input type="password" name="confirm_password" placeholder="Confirm new password" disabled>
							...
							<div style="text-align:right;margin-top:8px;">
								<button type="button" id="setNewPasswordBtn" style="display:none;background:#0ea5a4;color:#fff;border:none;padding:8px 10px;border-radius:6px;cursor:pointer;">Set New Password</button>
							</div>
				</div>
			</div>
			<div style="text-align:right;margin-top:8px;">
			...
			(function(){
				...
				function exitEdit(restore=true) {
					inputs.forEach(function(i){
						if (restore && original.hasOwnProperty(i.name)) {
				...
				// Attach edit handlers for each field edit button
				document.querySelectorAll('.field-edit').forEach(function(btn){
					btn.addEventListener('click', function(e){
						var field = btn.getAttribute('data-field');
						var input = form.querySelector('input[name="' + field + '"]');
						if (!input) return;
						var saveBtn = form.querySelector('.field-save[data-field="' + field + '"]');
						var cancelBtn = form.querySelector('.field-cancel[data-field="' + field + '"]');
				...
				if (editPassword) {
					editPassword.addEventListener('click', function(e){
						...
						// reveal password section for entering current password
								passwordSection.style.display = 'block';
								// ensure current-password input group is visible when entering edit mode
								var _cur = form.querySelector('input[name=current_password]');
								if (_cur) { var _g = _cur.closest('.form-group'); if (_g) _g.style.display = ''; }
				...
				if (cancelPassword) {
					cancelPassword.addEventListener('click', function(e){
						...
						curPwd.disabled = true; newPwd.disabled = true; confPwd.disabled = true;
								...
								// ensure current-password input group is visible again for next edit
								var _curShow = form.querySelector('input[name=current_password]'); if (_curShow) { var _g2 = _curShow.closest('.form-group'); if (_g2) _g2.style.display = ''; }
